    ...
    return (
      <Link href={href}>
        <a aria-current={active ? 'page' : undefined} aria-label={label || href} className="p-1 -m-1 nav-item-link">
          {active ? (
            <span className={`inline-flex items-center gap-2 xs:gap-3 bg-white/90 text-black rounded-full px-3 xs:px-4 py-1.5 xs:py-2 border border-white/70 shadow-lg ${shouldAnimate ? 'nav-item-active' : 'nav-item-static'}`}>
              <span className={`inline-flex items-center justify-center w-4 xs:w-5 h-4 xs:h-5 ${shouldAnimate ? 'nav-icon-active' : ''}`}>{children}</span>
              {label && <span className="font-medium text-xs xs:text-sm leading-none">{label}</span>}
            </span>
          ) : (
            <span className="inline-flex items-center justify-center w-9 xs:w-10 h-9 xs:h-10 text-white/80 rounded-full hover:bg-white/10 transition nav-item-inactive">
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 flex justify-center pointer-events-none pb-[calc(1.5rem+env(safe-area-inset-bottom))]" role="navigation" aria-label="Primary">
      <div className="pointer-events-auto w-full max-w-lg px-4">
        <div className="mx-auto bg-white/7 backdrop-blur-xl border border-white/15 rounded-full px-3 xs:px-4 py-1.5 xs:py-2 flex items-center justify-between gap-3 xs:gap-6 shadow-[0_10px_30px_rgba(0,0,0,0.4)]">
          ...
          <Item href="/workouts" label="Workouts">
            <svg className="w-4 xs:w-5 h-4 xs:h-5" viewBox="0 0 48 48" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
          ...
          <Item href="/statistics" label="Statistics">
            <svg className="w-4 xs:w-5 h-4 xs:h-5" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
          ...
          <Item href="/settings" label="Settings">
            <svg className="w-4 xs:w-5 h-4 xs:h-5" viewBox="0 0 416 432" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
