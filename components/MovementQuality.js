...
export default function MovementQuality({
  ...
  if (loading) {
    return (
      <div className="w-full bg-white/5 rounded-2xl p-3 xs:p-4 animate-pulse">
        <div className="flex items-center justify-between mb-3 xs:mb-4">
          <div className="h-5 w-32 xs:w-40 bg-white/10 rounded"></div>
          <div className="flex gap-1.5 xs:gap-2">
            {[1, 2, 3].map((i) => (
              <div key={i} className="h-7 w-12 xs:h-8 xs:w-16 bg-white/10 rounded-full"></div>
            ))}
          </div>
        </div>
        <div className="flex items-center gap-4 xs:gap-6">
          <div className="w-24 h-24 xs:w-32 xs:h-32 bg-white/10 rounded-full"></div>
  ...
  if (!hasData) {
    return (
      <div className="w-full">
        <div className="relative overflow-hidden bg-white/5 rounded-2xl p-3 xs:p-4">
          {/* Header */}
          <div className="flex items-center justify-between gap-3 mb-3 xs:mb-4 relative z-10">
            <div>
              <h3 className="text-lg xs:text-xl font-semibold text-white">Movement Quality</h3>
          ...
          <div className="flex flex-col items-center justify-center py-6 xs:py-8">
            <div className="text-white/30 mb-2">
              <svg className="w-10 xs:w-12 h-10 xs:h-12" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
              </svg>
            </div>
            <p className="text-xs xs:text-sm text-white/40">No data available</p>
  ...
  return (
    <div className="w-full">
      {/* Card Container */}
      <div className="relative overflow-hidden bg-white/5 rounded-2xl p-3 xs:p-4 transition-all duration-300">
        ...
        <div className="flex items-center justify-between gap-2 xs:gap-3 mb-3 xs:mb-4 relative z-10">
          {/* Title - No icon */}
          <div>
            <h3 className="text-lg xs:text-xl font-semibold text-white">Movement Quality</h3>
            <p className="text-[10px] text-white/40">Weekly aggregated result</p>
          ...
          <div className="flex gap-1.5">
            {filters.map((filter) => (
              <button
                ...
                className={`
                  p-2 xs:p-2.5 rounded-lg transition-all duration-300 ease-out
        ...
        <div className="flex items-center gap-3 xs:gap-5 relative z-10">
          ...
          <div className="relative flex-shrink-0">
            <div className="relative w-24 xs:w-28 h-24 xs:h-28">
              {/* Background circle */}
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span 
                  className={`text-2xl xs:text-3xl font-bold transition-all duration-300 ${isAnimating ? 'scale-105' : 'scale-100'}`}
