...
/**
 * Dumbbell Equipment Page
 * Shows workout history and stats for dumbbell exercises
 */
export default function DumbbellPage() {
  const router = useRouter()
  const { logs, loading } = useWorkoutLogs({ autoFetch: true, limitCount: 500 })

  // Filter logs for dumbbell exercises
  const dumbbellLogs = useMemo(() => {
    if (!logs || logs.length === 0) return []
    return logs.filter((log) => {
      const equipment = log.exercise?.equipmentPath || log.exercise?.equipment || ''
      return equipment.toLowerCase().includes('dumbbell')
    })
  }, [logs])

  // Calculate stats
  const stats = useMemo(() => {
    const totalSessions = dumbbellLogs.length
    const totalReps = dumbbellLogs.reduce((sum, log) => {
      return sum + (log.results?.totalReps || log.results?.completedReps || 0)
    }, 0)
    const totalLoad = dumbbellLogs.reduce((sum, log) => {
      const reps = log.results?.totalReps || log.results?.completedReps || 0
      const weight = log.exercise?.weight || 10
  ...
  return (
    <>
      <Head>
        <title>Dumbbell Exercises | AppLift</title>
      </Head>
      ...
      <main className="min-h-screen bg-[#0a0a0a] text-white pb-24">
        ...
        <header className="sticky top-0 z-10 bg-[#0a0a0a]/95 backdrop-blur-sm border-b border-white/5">
          <div className="px-4 py-4 flex items-center gap-4">
            ...
            <h1 className="text-xl font-bold">Dumbbell</h1>
          </div>
        </header>
          ...
          <section>
            ...
            {loading ? (
              ...
              <div className="bg-white/[0.05] rounded-2xl p-8 text-center">
                <p className="text-white/50">No dumbbell exercises recorded yet</p>
