...
/**
 * Weight Stack Equipment Page
 * Shows workout history and stats for weight stack/cable machine exercises
 */
export default function WeightStackPage() {
  const router = useRouter()
  const { logs, loading } = useWorkoutLogs({ autoFetch: true, limitCount: 500 })

  // Filter logs for weight stack exercises
  const weightStackLogs = useMemo(() => {
    if (!logs || logs.length === 0) return []
    return logs.filter((log) => {
      const equipment = log.exercise?.equipmentPath || log.exercise?.equipment || ''
      return equipment.toLowerCase().includes('weight') || 
             equipment.toLowerCase().includes('cable') ||
             equipment.toLowerCase().includes('machine') ||
             equipment.toLowerCase().includes('stack')
    })
  }, [logs])

  // Calculate stats
  const stats = useMemo(() => {
    const totalSessions = weightStackLogs.length
    const totalReps = weightStackLogs.reduce((sum, log) => {
      return sum + (log.results?.totalReps || log.results?.completedReps || 0)
    }, 0)
    const totalLoad = weightStackLogs.reduce((sum, log) => {
      const reps = log.results?.totalReps || log.results?.completedReps || 0
      const weight = log.exercise?.weight || 30
  ...
  return (
    <>
      <Head>
        <title>Weight Stack Exercises | AppLift</title>
      </Head>
      ...
      <main className="min-h-screen bg-[#0a0a0a] text-white pb-24">
        ...
        <header className="sticky top-0 z-10 bg-[#0a0a0a]/95 backdrop-blur-sm border-b border-white/5">
          <div className="px-4 py-4 flex items-center gap-4">
            ...
            <h1 className="text-xl font-bold">Weight Stack</h1>
          </div>
        </header>
          ...
          <section>
            ...
            {loading ? (
              ...
              <div className="bg-white/[0.05] rounded-2xl p-8 text-center">
                <p className="text-white/50">No weight stack exercises recorded yet</p>
