...
const computePhaseTimings = (accelX, accelY, accelZ, timestamps) => {
  ...
  // *** SWAP: For acceleration-based detection, the "peak" is the max acceleration spike
  // which occurs DURING the initial lift, NOT at the physical turning point.
  // So "before peak" is actually a tiny early part of concentric, and "after peak"
  // includes the rest of concentric + all of eccentric.
  // Swap to match the orientation-based convention where liftingTime = concentric, loweringTime = eccentric.
  return { 
    liftingTime: loweringTime, 
    loweringTime: liftingTime, 
    primaryAxis,
    peakTimePercent: 100 - peakTimePercent
  };
};
...
export const computeFatigueIndicators = (repMetricsList, mlClassification = null) => {
  ...
  // Use a minimum floor for baseline to prevent extreme ratios when first reps are "too clean"
  const JERK_FLOOR = 2.0;  // Minimum baseline jerk (rad/s³) — typical clean rep is 2-5
  ...
  // Use a minimum floor for baseline to prevent extreme ratios when first reps are "too clean"
  const SHAKINESS_FLOOR = 1.0;  // Minimum baseline shakiness (rad/s²) — typical clean rep is 1-3
  let I_S = 0;
  let avgShakyFirst = 0, avgShakyLast = 0;
  if (hasShakiness) {
    avgShakyFirst = Math.max(SHAKINESS_FLOOR, mean(shakiness.slice(0, third)));
